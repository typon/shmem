...
NB_MODULE(cyshmem, m) {
    ...
    nb::class_<shmem::SMQueue>(m, "SMQueue")
        ...
        // Custom implementation for pop that returns generic arrays
        .def(
            "pop_np",
            [](shmem::SMQueue& self) {
                ...
                self.pop(reinterpret_cast<std::byte*>(data));
        ...
        .def(
            ...
            [](shmem::SMQueue& self) -> std::optional<nb::ndarray<nb::numpy, uint8_t>> {
                ...
                // Allocate memory for the array directly (single buffer)
                uint8_t* data = new uint8_t[size];
                
                // Try to pop data directly into the allocated memory
                ...
                auto result = nb::ndarray<nb::numpy, uint8_t>(
                    data, 
                    shape.size(), 
                    shape.data(), 
                    deleter, 
