...
bool SMQueue::push(const std::byte* data) {
    ...
    // Copy the message directly - use memcpy for large blocks
...
void SMQueue::pop(std::byte* buffer) {
    if (!m_addr) {
        return;
    }
    ...
    // Copy the message from the queue at the tail position
    std::memcpy(buffer, get_element(cb->tail), cb->element_size);
...
std::size_t SMQueue::max_elements() const { return m_addr ? get_control_block()->max_elements : 0; }

// Get element size in bytes
std::size_t SMQueue::element_size() const { return m_addr ? get_control_block()->element_size : 0; }
